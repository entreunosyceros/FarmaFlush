...
from config import BASE_DIR, SECRET_KEY, DEBUG, LOG_LEVEL
...
def _configure_logging() -> None:
    """Configura el nivel de log de la app sin inundar la consola con peticiones HTTP."""
...
def _enriquecer_desde_farmacias(med: dict, complemento: dict | None = None) -> None:
    ...
    # Priorizar SIEMPRE la consulta original del usuario cuando exista (marca / nombre comercial).
    # Esto evita que productos con el mismo principio activo "ensucien" la comparativa
    # (ej: "Aspirina" → no queremos "Couldina instant" solo por llevar AAS).
    qorig = (med.get("_query_original") or "").strip()
    # 1) Intento primario: marca/consulta original (si existe)
    # 2) Fallback: primera alternativa (principio activo/dosis) SOLO si el intento primario no da ofertas con precio.
    marca_laboratorio = med.get("laboratorio") or med.get("laboratorio_comercializador") or ""

    def _buscar(query: str, extras_q: list[str]):
        return farmacia.buscar_ofertas(
            query,
            marca=marca_laboratorio,
            max_sources=len(farmacia.FUENTES_FARMACIA),
            consultas_extra=extras_q,
            incluir_sin_resultado=True,
        )

    comparativa = []
    med["farmacias_aviso_fallback"] = ""
    med["farmacias_query_usada"] = ""

    if 2 <= len(qorig) <= 64:
        consulta_principal = qorig
        extras_prim = [c for c in consultas_extra if c.strip().lower() != qorig.lower()]
        comparativa = _buscar(consulta_principal, extras_prim)
        med["farmacias_query_usada"] = consulta_principal

        precios_online = [o.get("precio") for o in comparativa if o.get("precio") is not None]
        if not precios_online:
            # Fallback: usar alternativa (si existe) y avisar
            fallback_query = (consultas_extra[0] if consultas_extra else "").strip()
            if fallback_query and fallback_query.lower() != qorig.lower():
                comparativa = _buscar(fallback_query, [c for c in consultas_extra[1:] if c.strip()])
                med["farmacias_query_usada"] = fallback_query
                med["farmacias_aviso_fallback"] = (
                    "Aviso: no se encontraron ofertas por nombre comercial; se muestran resultados relacionados por principio activo."
                )
    else:
        consulta_principal = consultas_extra[0] if consultas_extra else med["nombre"]
        extras = consultas_extra[1:] if len(consultas_extra) > 1 else []
        comparativa = _buscar(consulta_principal, extras)
        med["farmacias_query_usada"] = consulta_principal
    ...
    if (not med.get("imagen_url") or med.get("imagen_url", "").endswith("product-placeholder.svg")):
        producto_relacionado = farmacia.buscar_producto_relacionado(
            med.get("farmacias_query_usada") or consulta_principal,
...
def _procesar_ticket_items(nombres: list[str], precios_raw: list[str]) -> tuple[list[dict], dict | None]:
    """Procesa líneas de ticket y devuelve items + resumen. Reutilizado en verificar-ticket y PDF."""
    items: list[dict] = []
...
def verificar_ticket():
    ...
    if request.method == "POST":
        items, resumen = _procesar_ticket_items(
            request.form.getlist("q"),
...
@app.route("/verificar-ticket/informe.pdf", methods=["POST"])
def verificar_ticket_informe_pdf():
    """Genera el Informe de Auditoría Ciudadana en PDF a partir del ticket enviado."""
    from services.informe_auditoria import generar_informe_pdf
    ...
    if not resumen:
        return render_template("ticket.html", items=items, resumen=None), 400

    pdf_bytes = generar_informe_pdf(items, resumen)
    fecha = time.strftime("%Y%m%d")
    response = make_response(pdf_bytes)
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f'attachment; filename="farmaflush-informe-auditoria-{fecha}.pdf"'
    )
    return response
...
if __name__ == "__main__":
    _port = 5000
    _host = "0.0.0.0"
    print(f"\n✅ FarmaFLUSH listo en http://127.0.0.1:{_port}/  (Ctrl+C para detener)\n", flush=True)
    app.run(debug=DEBUG, host=_host, port=_port)
