...
Este proyecto nace de una necesidad real y personal: garantizar que el derecho a la salud no se vea afectado por "errores" en el redondeo o sobreprecios aplicados a medicamentos con precio regulado. FarmaFLUSH utiliza datos abiertos y técnicas de agregación de datos para ofrecer transparencia inmediata al ciudadano, por que en los tiempos que corren, la diferencia de precios entre un sitio y otro puede ser muy grande.

**~~Demo Online~~**

~~El proyecto está desplegado actualmente en: <https://farmaflush.pythonanywhere.com>~~

> ⚠️ **Nota importante:** La demo online ya no está disponible debido a que el servidor gratuito de PythonAnywhere no pudo soportar las visitas. Al lanzar tantas peticiones en paralelo para scrapear las farmacias a la vez, se consumieron todos los recursos del hosting. 
> 
> Ahora el proyecto está enfocado para ser **ejecutado en local** clonando este repositorio y usando los scripts de arranque. ¡Es más rápido y no dependemos de servidores externos!
> Si quieres y tienes dinero para ello, también puedes subirlo a un VPS.
---
...
| Fuentes online | Dosfarma · Farmacia Tedin · Farmacias Direct · Castrofarma · Farmacia Barata · FarmaGalicia · OpenFarma · Farmacia Pontevea · Gomezulla · Amazon.es |
...
| `LOG_LEVEL` | Nivel de log de la aplicación (`INFO`, `WARNING`, `DEBUG`…). | Por defecto `INFO`. Los logs de peticiones HTTP de Werkzeug/httpx solo se muestran en modo `DEBUG`. |
...
```bash
# Recomendado (Linux): el script crea .venv si no existe, instala dependencias y arranca
./run_app.sh
```

En Windows, utiliza `run_app.bat` (crea `.venv`, instala dependencias, libera el puerto si está ocupado y arranca la app).

**Nota sobre la consola:** al arrancar verás mensajes como `Serving Flask app` y `Running on http://127.0.0.1:5000`. No son errores. Las líneas `GET / ... 200` o `304` son peticiones normales del navegador (incluido el service worker). Con `LOG_LEVEL=INFO` (por defecto) no se imprimen en consola salvo que actives `FLASK_DEBUG=1` o `LOG_LEVEL=DEBUG`.
...
├── run_app.sh              # Arranque en Linux (crea .venv, instala deps, arranca)
├── run_app.bat             # Arranque en Windows (idem + libera puerto 5000)
...
│   ├── informe_auditoria.py         # Generación PDF Informe de Auditoría Ciudadana (modo ticket)
│   └── vademecum.py                 # Complemento informativo
├── assets/
│   └── fonts/                       # Fuentes DejaVu para PDF (multiplataforma)
├── static/
│   └── img/
├── templates/
...
- **Exportar Informe de Auditoría Ciudadana (PDF):** tras verificar, genera un documento formal con ID único, resumen ejecutivo, tabla de desfases, anexo de líneas correctas, metodología, aviso legal y checklist. Útil en mostrador o reclamaciones.
...
- Búsqueda paralela en múltiples fuentes (farmacias online + marketplace)
...
- En cada resultado se muestra también la **ubicación** de la fuente (provincia/ciudad o `Nacional` cuando no aplica), p. ej. `Dosfarma (Murcia — Cartagena)`

**Ubicación mostrada por fuente** (orientativa; farmacias online con sede física de referencia):
...
- **Aviso de instalación PWA solo en móvil:** el mensaje del pie «Instálame en tu móvil…» y el banner de instalación solo aparecen en dispositivos táctiles; en escritorio no se muestran
...
**Nota sobre marca vs principio activo (farmacias online):** cuando el usuario busca por una **marca** (nombre comercial), el sistema intenta primero esa búsqueda en farmacias online. Si no encuentra ofertas por nombre comercial, hace un **fallback** a una búsqueda por **principio activo/dosis** y muestra un **aviso visible** indicando que los resultados pueden ser productos relacionados y no exactamente la misma marca o presentación.
...
| **Gomezulla en tu Piel** | Prestashop | Cosmética dermatológica |
| **Amazon.es** | Marketplace | Parafarmacia y cosmética (múltiples vendedores) |
...
- [x] Scripts de arranque multiplataforma (`run_app.sh`, `run_app.bat`) con creación automática de `.venv`
- [x] Ubicación de farmacia online visible en resultados (provincia/ciudad)
- [x] Búsqueda en farmacias online priorizando marca; fallback a principio activo con aviso visible
- [x] Aviso de instalación PWA limitado a dispositivos móviles
- [x] Modo ticket: exportar **Informe de Auditoría Ciudadana** en PDF (resumen ejecutivo, tabla de desfases, metodología y disclaimer; útil en mostrador o reclamaciones)
