...
# Ubicación asociada a la fuente (si la conocemos), para mostrar en tarjetas:
# "Farmacia X (Provincia — Ciudad)" o "Amazon.es (Nacional)".
# Si está vacío/ausente, no se muestra etiqueta.
_PROVEEDOR_PROVINCIA: dict[str, str] = {
    farmacia_dosfarma.FUENTE_ID: "Murcia — Cartagena",
    farmacia_tedin.FUENTE_ID: "A Coruña — Santiago de Compostela",
    farmacia_farmaciasdirect.FUENTE_ID: "Sevilla — Sevilla",
    farmacia_castrofarma.FUENTE_ID: "Lugo — Castro de Rei",
    farmacia_farmaciabarata.FUENTE_ID: "Madrid/Almería — Madrid (Vera)",
    farmacia_farmagalicia.FUENTE_ID: "Lugo — Monforte de Lemos",
    farmacia_openfarma.FUENTE_ID: "Alicante — Alicante",
    farmacia_pontevea.FUENTE_ID: "A Coruña — Teo (Pontevea)",
    farmacia_gomezulla.FUENTE_ID: "Madrid — Madrid",
    farmacia_amazon.FUENTE_ID: "Nacional",
}
...
FUENTES_FARMACIA = [
    {
        ...
        "provincia": _PROVEEDOR_PROVINCIA.get(proveedor.FUENTE_ID, ""),
    }
    for proveedor in _PROVEEDORES
]


def _anotar_provincia_fuente(producto: dict) -> dict:
    """Añade la provincia de la fuente al producto (si se conoce)."""
    if not isinstance(producto, dict):
        return producto
    fid = (producto.get("fuente") or "").strip()
    if fid:
        provincia = _PROVEEDOR_PROVINCIA.get(fid, "")
        if provincia:
            producto["fuente_provincia"] = provincia
    return producto


def _anotar_provincia_oferta(oferta: dict) -> dict:
    """Añade la ubicación de la fuente a una oferta (si se conoce)."""
    if not isinstance(oferta, dict):
        return oferta
    fid = (oferta.get("fuente") or "").strip()
    if fid:
        provincia = _PROVEEDOR_PROVINCIA.get(fid, "")
        if provincia:
            oferta["fuente_provincia"] = provincia
    return oferta
    ...
    for proveedor in _PROVEEDORES:
        ...
        for termino in _TERMINOS_PARAFARMACIA:
            ...
            for producto in productos_proveedor:
                ...
                producto = _anotar_provincia_fuente(producto)
...
def buscar_ofertas(
    ...
    with ThreadPoolExecutor(max_workers=len(proveedores)) as executor:
        futuros = {executor.submit(_buscar_desde_proveedor, prov): prov for prov in proveedores}
    ...
    for oferta in ofertas:
        _anotar_provincia_oferta(oferta)
    ...
    if incluir_sin_resultado:
        for prov in proveedores:
            if prov.FUENTE_ID not in fuentes_con_resultado:
                ofertas.append(_anotar_provincia_oferta({
